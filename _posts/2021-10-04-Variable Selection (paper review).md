...
Consider a model $Y=X\beta $ with $X \in \mathbb{R}^{n\time p}$ and $\beta \in \mathbb{R}^{p}$. Variable selection has been highly compelling when $n << p$ and many of componenets in $\beta$ are not significant, which is called a sparse linear model. Along with interests of this field, diverse approaches have been suggested such as stepwise-selection, LASSO, etc.. 

What this paper especially works on is a 'sparse prior' whose structure consists of the following steps.

1. Select dimension of $\beta$: $s \sim \pi_{p}$ on the set $\{0,1,2,\dots,p\}$
